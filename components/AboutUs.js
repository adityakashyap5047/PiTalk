...
const AboutUs = () => {
  ...
  return (
    <section className="py-10 bg-gradient-linear from-[#080F58] via-purple-500 to-[#080F58] bg-[length:200%_200%] animate-bg-gradient dark:from-[#121212] dark:via-[#080F58] dark:to-[#121212]">
      ...
      <h3 className="text-5xl text-[#0E0E5C] dark:text-[#4441d5] font-semibold text-center flex gap-2 justify-center mb-10">
        ...
        <div className="space-y-4">
          {faqData.map((faq, index) => (
            <div
              key={index}
              className="bg-sky-700 shadow-md rounded-lg p-4 transition hover:shadow-lg dark:bg-[#23224f]"
            >
              <div
                className="flex justify-between items-center cursor-pointer"
                onClick={() => toggleAnswer(index)}
              >
                <h3 className="text-lg font-semibold text-gray-800 dark:text-[#8ec0e8]">
                  {faq.question}
                </h3>
                {activeIndex === index ? (
                  <FiChevronUp className="text-white dark:text-[#c5b8e8]" />
                ) : (
                  <FiChevronDown className="text-white dark:text-[#c5b8e8]" />
                )}
              </div>
              {activeIndex === index && (
                <p className="mt-4 text-[#8ec0e8] dark:text-[#c5b8e8]">{faq.answer}</p>
              )}
            </div>
          ))}
        </div>
